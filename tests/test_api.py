...
def test_approve_best_launch_ready_canary_proposal_saves_top_ready_snapshot(
    ...
    async def fake_scan_live_route_candidate_for_approval(
        **kwargs: object,
        ...
        approval = cast(RouteApprovalEntry, kwargs["approval"])
        assert approval.label == "arb_extended_paradex"
        assert approval.approved is True
        return _arb_extended_paradex_canary_candidate(suggested_canary_notional=25.0), 1
    ...
    try:
        ...
        response = client.post(
            ...
            params={"use_history_shortlist": "false", "candidate_sample": "10"},
        )
    finally:
        app.dependency_overrides.clear()
    ...
    assert approval_service.list_recent(label="arb_extended_paradex")[0].approved is True
    assert approved_store.latest(label="arb_extended_paradex") is not None
    assert launch_ready_store.latest(label="arb_extended_paradex") is not None
...
def test_execution_pair_close_preview_confirmation_endpoint_rejects_stale_client_preview(
    ...
    assert response.status_code == 409
    assert "must have reason 'close_pair'" in response.json()["detail"]
    assert confirmation_store.list_recent(limit=10, paper_trade_id=paper_trade.entry_id) == []


def test_execution_pair_close_preview_confirmation_endpoint_accepts_valid_client_preview_without_rebuilding_canonical(  # noqa: E501
    tmp_path: Path,
) -> None:
    paper_store = PaperTradeStore(tmp_path / "paper.sqlite3")
    paper_trade = paper_store.append(
    ...
    provided_legs = [
        VenueOrderPreview(
            ...
            payload={
                ...
                "price": "0.08840000",
                "instruction": "IOC",
                "client_id": "carryme-cleanup-pt10-paradex-sell",
                "reduce_only": True,
            },
            notes=["client"],
        ),
        VenueOrderPreview(
            ...
            payload={
                ...
                "post_only": False,
                "reduce_only": True,
                "client_order_id": "carryme-cleanup-pt10-buy",
            },
            notes=["client"],
        ),
    ]
...
def test_execution_pair_close_preview_confirmation_endpoint_rejects_client_preview_with_mismatched_execution_id(  # noqa: E501
    tmp_path: Path,
) -> None:
    paper_store = PaperTradeStore(tmp_path / "paper.sqlite3")
    paper_trade = paper_store.append(
        ...
        async def preview_from_execution(
            ...
            *,
            ...
            raise AssertionError(
                "server should not rebuild canonical preview when preview is provided"
            )
    ...
    bad_preview_hash = _pair_close_hash(
        execution_entry_id=(execution.entry_id or 0) + 999,
        paper_trade_id=paper_trade.entry_id,
        legs=provided_legs,
    )
    bad_preview = ExecutionPairClosePreview(
        execution_entry_id=(execution.entry_id or 0) + 999,
        paper_trade_id=paper_trade.entry_id,
        label="arb_extended_paradex",
        generated_at=datetime(2026, 3, 29, 13, 1, tzinfo=UTC),
        slippage_tolerance_bps=10,
        preview_hash=bad_preview_hash,
        reason="close_pair",
        legs=provided_legs,
        notes=["client supplied"],
    )
    ...
    response = client.post(
        f"/v1/executions/pair-close-preview-confirmations/latest/from-paper-trade/{paper_trade.entry_id}",
        params={"preview_hash": bad_preview_hash},
        json=bad_preview.model_dump(mode="json"),
    )
    app.dependency_overrides.clear()

    assert response.status_code == 409
    assert "must target the latest execution entry" in response.json()["detail"]
    assert confirmation_store.list_recent(limit=10, paper_trade_id=paper_trade.entry_id) == []
...
def test_execution_pair_close_preview_confirmation_endpoint_rejects_payload_leg_mismatch(
    tmp_path: Path,
) -> None:
    paper_store = PaperTradeStore(tmp_path / "paper.sqlite3")
    paper_trade = paper_store.append(
    ...
    preview_legs = [
        VenueOrderPreview(
            ...
            payload={
                ...
                "price": "0.08840000",
                "instruction": "IOC",
                "client_id": "carryme-cleanup-pt1-paradex-sell",
                "reduce_only": True,
            },
            notes=["client"],
        ),
        VenueOrderPreview(
            ...
            payload={
                ...
                "post_only": False,
                "reduce_only": True,
                "client_order_id": "carryme-cleanup-pt1-buy",
            },
            notes=["client"],
        ),
    ]
    ...
    response = client.post(
        f"/v1/executions/pair-close-preview-confirmations/latest/from-paper-trade/{paper_trade.entry_id}",
        params={"preview_hash": preview_hash},
        json=provided_preview.model_dump(mode="json"),
    )
    app.dependency_overrides.clear()
    ...
    assert response.status_code == 409
    assert (
        "Paradex pair close payload size did not match the preview leg"
...
def test_guarded_paired_live_execution_endpoint_chains_auto_cleanup_until_closed(
    tmp_path: Path,
) -> None:
    paper_store = PaperTradeStore(tmp_path / "history.sqlite3")
    confirmation_store = PreviewConfirmationStore(tmp_path / "history.sqlite3")
    ...
    paper_trade = paper_store.append(
        PaperTradeEntry(
            created_at=datetime(2026, 4, 2, 18, 49, tzinfo=UTC),
            note="operator accepted candidate",
            intent=FundingPairTradeIntent(
                label="jup_extended_paradex",
                canonical_symbol="JUP-USD-PERP",
                source_recorded_at=datetime(2026, 4, 2, 18, 48, tzinfo=UTC),
                one_day_net_edge_after_entry=0.001,
                break_even_days_entry=0.5,
    ...
    confirmation_store.append(
        PreviewConfirmationEntry(
            confirmed_at=datetime(2026, 4, 2, 18, 49, tzinfo=UTC),
            paper_trade_id=paper_trade.entry_id or 0,
            label="jup_extended_paradex",
            preview_hash="preview-hash",
            preview=PaperTradeOrderPreview(
                paper_trade_id=paper_trade.entry_id or 0,
                label="jup_extended_paradex",
                generated_at=datetime(2026, 4, 2, 18, 49, tzinfo=UTC),
                ...
                legs=[
                    VenueOrderPreview(
                        ...
                        minimum_notional=10.0,
                        reference_price=0.1567,
                        reference_price_source="best_ask",
                        worst_acceptable_price=0.1568,
                        worst_price_text="0.1568",
                        price_increment=0.0001,
                        max_order_value=1_000_000.0,
                        endpoint_path_hint="/v1/orders",
                        auth_scheme="main account address + subkey private key",
                        payload={"market": "JUP-USD-PERP"},
                        notes=[],
                    ),
                    VenueOrderPreview(
                        venue="extended",
                        ...
                        minimum_order_size=1.0,
                        minimum_notional=10.0,
                        reference_price=0.1562,
                        reference_price_source="best_bid",
                        worst_acceptable_price=0.1561,
                        worst_price_text="0.1561",
                        price_increment=0.0001,
                        max_order_value=1_000_000.0,
                        endpoint_path_hint="/api/v1/user/order",
                        auth_scheme="api key + Stark signing key",
                        payload={"symbol": "JUP-USD"},
                        notes=[],
                    ),
                ],
            ),
    ...
    class StubAccountPreflightService:
        ...
        async def probe_paper_trade(
            ...
            self.probe_paper_trade_calls += 1
            if self.probe_paper_trade_calls == 1:
                extended_positions: list[str] = []
                paradex_positions = []
            elif self.probe_paper_trade_calls == 2:
                extended_positions = []
                paradex_positions = ["JUP-USD-PERP"]
            elif self.probe_paper_trade_calls == 3:
                extended_positions = ["JUP-USD"]
                paradex_positions = []
            else:
                extended_positions = []
                paradex_positions = []
            return PaperTradeAccountPreflight(
                ...
                venues=[
                    VenueAccountPreflight(
                        ...
                        position_symbols=extended_positions,
                    ),
                    VenueAccountPreflight(
                        venue="paradex",
                        enabled=True,
                        ...
                        credential_mode="subkey_jwt",
                        free_collateral=500.0,
                        position_symbols=paradex_positions,
                    ),
                ],
                blocking_reasons=[],
            )
    ...
    class StubExecutionOrderStateService:
        async def observe_execution(self, entry: ExecutionJournalEntry) -> ExecutionOrderState:
            if entry.adapter.startswith("paired_live:"):
                return ExecutionOrderState(
                    execution_entry_id=entry.entry_id,
                    paper_trade_id=entry.paper_trade_id,
                    preview_hash=entry.preview_hash,
                    legs=[
                        ExecutionLegOrderState(
                            venue="paradex",
                            supported=True,
                            external_reference="pdx-open",
                            derived_state="filled",
                        ),
                        ExecutionLegOrderState(
                            venue="extended",
                            supported=True,
                            external_reference="ext-open",
                            derived_state="unknown",
                        ),
                    ],
                )
            venue = entry.legs[0].venue
            return ExecutionOrderState(
                execution_entry_id=entry.entry_id,
                paper_trade_id=entry.paper_trade_id,
                preview_hash=entry.preview_hash,
                legs=[
                    ExecutionLegOrderState(
                        venue=venue,
                        supported=True,
                        external_reference=f"{venue}-cleanup",
                        derived_state="filled",
                    )
                ],
            )
    ...
    class StubPairedLiveExecutionCoordinator:
        async def submit_confirmed_preview(
            ...
            *,
            ...
            return ExecutionJournalEntry(
                executed_at=datetime(2026, 4, 2, 18, 49, 6, tzinfo=UTC),
                adapter=f"paired_live:{first_venue}_then_extended",
                mode="live",
                status="submitted",
                paper_trade_id=paper_trade.entry_id,
                preview_hash=confirmation.preview_hash,
    ...
    class StubCleanupPreviewService:
        ...
        async def preview_from_execution(
            ...
            *,
            ...
            self.calls.append(entry.adapter)
            if entry.adapter.startswith("paired_live:"):
                assert pair_status.recommended_action == "complete_or_unwind_missing_leg"
                return ExecutionCleanupPreview(
                    execution_entry_id=entry.entry_id,
                    paper_trade_id=paper_trade.entry_id or 0,
                    generated_at=datetime(2026, 4, 2, 18, 49, 10, tzinfo=UTC),
                    preview_hash="cleanup-hash-1",
                    reason="complete_or_unwind_missing_leg",
                    leg=VenueOrderPreview(
                        venue="paradex",
                        ...
                        quantity=159.0,
                        quantity_text="159",
                        reference_price=0.1554,
                        reference_price_source="best_bid",
                        worst_acceptable_price=0.1554,
                        worst_price_text="0.1554",
                        reduce_only=True,
                        endpoint_path_hint="/v1/orders",
                        auth_scheme="main account address + subkey private key",
                        payload={"market": "JUP-USD-PERP", "reduce_only": True},
                        notes=[],
                    ),
                    notes=[],
                )
            ...
            return ExecutionCleanupPreview(
                execution_entry_id=entry.entry_id,
                paper_trade_id=paper_trade.entry_id or 0,
                generated_at=datetime(2026, 4, 2, 18, 49, 20, tzinfo=UTC),
                preview_hash="cleanup-hash-2",
                reason="close_open_leg",
                leg=VenueOrderPreview(
                    venue="extended",
                    ...
                    quantity=160.0,
                    quantity_text="160",
                    reference_price=0.1657,
                    reference_price_source="best_ask",
                    worst_acceptable_price=0.1657,
                    worst_price_text="0.1657",
                    reduce_only=True,
                    endpoint_path_hint="/api/v1/user/order",
                    auth_scheme="api key + Stark signing key",
                    payload={"symbol": "JUP-USD", "reduce_only": True},
                    notes=[],
                ),
                notes=[],
            )
    ...
    class StubCleanupLiveExecutionRouter:
        ...
        async def submit_confirmed_cleanup_preview(
            ...
            *,
            ...
            self.calls.append(confirmation.preview.leg.venue)
            adapter = f"{confirmation.preview.leg.venue}_cleanup_live"
            return ExecutionJournalEntry(
                executed_at=datetime(2026, 4, 2, 18, 49, 15 + len(self.calls), tzinfo=UTC),
                adapter=adapter,
                mode="live",
                status="submitted",
                paper_trade_id=paper_trade.entry_id,
                ...
                legs=[
                    ExecutionLegResult(
                        ...
                        symbol=confirmation.preview.leg.symbol,
                        fee_profile=confirmation.preview.leg.fee_profile,
                        side=confirmation.preview.leg.side,
                        target_notional=confirmation.preview.leg.target_notional,
                        status="submitted",
                        simulated=False,
                        external_reference=f"cleanup-{len(self.calls)}",
                        request_payload=confirmation.preview.leg.payload,
                    )
                ],
            )
    ...
    account_service = StubAccountPreflightService()
    cleanup_preview_service = StubCleanupPreviewService()
    cleanup_live_router = StubCleanupLiveExecutionRouter()
    app.dependency_overrides[get_paper_trade_store] = lambda: paper_store
    app.dependency_overrides[get_preview_confirmation_store] = lambda: confirmation_store
    app.dependency_overrides[get_cleanup_preview_confirmation_store] = (
        lambda: cleanup_confirmation_store
    ...
    app.dependency_overrides[get_route_approval_service] = lambda: _AllowAllRouteApprovalService()
    app.dependency_overrides[get_cleanup_preview_service] = lambda: cleanup_preview_service
    app.dependency_overrides[get_cleanup_live_execution_router] = (
        lambda: cleanup_live_router
    )
    app.dependency_overrides[get_execution_observation_store] = lambda: observation_store
    app.dependency_overrides[get_api_settings] = lambda: ApiSettings(
        extended_live_enabled=True,
    ...
    assert response.status_code == 200
    payload = response.json()
    assert payload["pair_status"]["derived_state"] == "closed"
    assert payload["pair_status"]["recommended_action"] == "no_action"
    assert payload["cleanup_execution"]["adapter"] == "extended_cleanup_live"
    assert cleanup_live_router.calls == ["paradex", "extended"]
    cleanup_confirmations = cleanup_confirmation_store.list_recent(limit=10)
    assert [entry.preview_hash for entry in cleanup_confirmations] == [
        "cleanup-hash-2",
        "cleanup-hash-1",
    ]
    saved_executions = sorted(
        execution_store.list_recent(limit=10),
        key=lambda entry: entry.executed_at,
    )
    assert [entry.adapter for entry in saved_executions] == [
        "paired_live:paradex_then_extended",
        "paradex_cleanup_live",
...
def test_guarded_paired_live_execution_endpoint_rejects_cleanup_reservation_without_execution(
    tmp_path: Path,
) -> None:
    paper_store = PaperTradeStore(tmp_path / "history.sqlite3")
    confirmation_store = PreviewConfirmationStore(tmp_path / "history.sqlite3")
    ...
    class StubAccountPreflightService:
        ...
        async def probe_paper_trade(
            ...
            self.probe_paper_trade_calls += 1
            paradex_positions = (
                [] if self.probe_paper_trade_calls == 1 else ["JUP-USD-PERP"]
            )
            return PaperTradeAccountPreflight(
                paper_trade_id=paper_trade.entry_id or 0,
                label=paper_trade.intent.label,
    ...
    class StubCleanupLiveExecutionRouter:
        async def submit_confirmed_cleanup_preview(
            ...
            *,
            ...
            raise AssertionError(
                "cleanup router should not be called when reservation already exists"
            )

    stale_cleanup_confirmation = cleanup_confirmation_store.append(
        CleanupPreviewConfirmationEntry(
            confirmed_at=datetime(2026, 4, 2, 18, 49, 11, tzinfo=UTC),
            paper_trade_id=paper_trade.entry_id or 0,
            label=paper_trade.intent.label,
            preview_hash="cleanup-hash-1",
            preview=ExecutionCleanupPreview(
                execution_entry_id=confirmation.entry_id,
                paper_trade_id=paper_trade.entry_id or 0,
                generated_at=datetime(2026, 4, 2, 18, 49, 10, tzinfo=UTC),
                preview_hash="cleanup-hash-1",
                reason="complete_or_unwind_missing_leg",
    ...
    assert stale_cleanup_confirmation.entry_id is not None
    assert execution_store.reserve_live_submission(
        confirmation_entry_id=stale_cleanup_confirmation.entry_id,
        preview_hash=stale_cleanup_confirmation.preview_hash,
    )
    ...
    assert response.status_code == 409
    assert "reserved without a matching journaled execution" in response.json()["detail"]
