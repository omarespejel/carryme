...
def test_worker_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert settings.execution_auto_pair_close_min_profit_total_collateral is None
...
def _build_stable_launch_test_snapshot(
    *,
    ...
    min_daily_volume: float = 110_000.0,
    deployable_notional: float = 900.0,
    estimated_one_day_pnl_after_round_trip: float = 2.79,
...
@pytest.mark.parametrize(
    ("latest_outcome", "cleanup_needed_count", "review_required_count"),
    (
        ("review_required", 0, 1),
        ("cleanup_needed", 1, 0),
    ),
...
def test_launch_latest_stable_canary_once_skips_when_latest_outcome_is_adverse(
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        ...
        monkeypatch.setattr(
            "carryme_worker.poller._run_guarded_canary_lifecycle",
            lambda **_: (_ for _ in ()).throw(
                AssertionError("latest adverse outcome must block before lifecycle launch")
            ),
        )
        summary = asyncio.run(
            launch_latest_stable_canary_once(
...
def test_launch_latest_stable_canary_once_allows_launch_when_latest_outcome_is_clean(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
    ...
    class StubLifecycleResult:
        def __init__(self) -> None:
            ...
            self.execution = ExecutionJournalEntry(
                ...
                status="submitted",
                paper_trade_id=118,
                preview_hash="launch-preview-latest-outcome",
...
def test_launch_latest_stable_canary_once_skips_when_min_daily_volume_requirement_not_met(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
        stable_canary_launch_min_daily_volume=50_000.0,
    )
    approval, candidate, snapshot, stability = _build_stable_launch_test_snapshot(
        label="arb_extended_paradex",
        min_daily_volume=9_230.9592,
    )
    approved_store = ApprovedCanaryStore(settings.database_path)
    persisted_snapshot = approved_store.append(snapshot.approved_snapshot)
    snapshot = snapshot.model_copy(update={"approved_snapshot": persisted_snapshot})
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        ...
        monkeypatch.setattr(
            "carryme_worker.poller._run_guarded_canary_lifecycle",
            lambda **_: (_ for _ in ()).throw(
                AssertionError("daily volume gate must block before lifecycle launch")
            ),
        )
        summary = asyncio.run(
            launch_latest_stable_canary_once(
    ...
    assert summary.detail == (
        ...
        "min_daily_volume=9230.9592 < min=50000.0000"
    )


def test_launch_latest_stable_canary_once_skips_when_deployable_notional_requirement_not_met(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
        stable_canary_launch_min_deployable_notional=500.0,
    )
    approval, candidate, snapshot, stability = _build_stable_launch_test_snapshot(
        label="arb_extended_paradex",
        deployable_notional=304.60986,
    )
    approved_store = ApprovedCanaryStore(settings.database_path)
    persisted_snapshot = approved_store.append(snapshot.approved_snapshot)
    snapshot = snapshot.model_copy(update={"approved_snapshot": persisted_snapshot})
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        ...
        monkeypatch.setattr(
            "carryme_worker.poller._run_guarded_canary_lifecycle",
            lambda **_: (_ for _ in ()).throw(
                AssertionError("deployable-notional gate must block before lifecycle launch")
            ),
        )
        summary = asyncio.run(
            launch_latest_stable_canary_once(
    ...
    assert summary.detail == (
        ...
        "deployable_notional=304.6099 < min=500.0000"
    )


def test_launch_latest_stable_canary_once_skips_when_expected_round_trip_pnl_requirement_not_met(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
        stable_canary_launch_min_expected_one_day_round_trip_pnl=0.07,
    )
    approval, candidate, snapshot, stability = _build_stable_launch_test_snapshot(
        label="arb_extended_paradex",
    ...
    with pytest.MonkeyPatch.context() as monkeypatch:
        ...
        monkeypatch.setattr(
            "carryme_worker.poller._run_guarded_canary_lifecycle",
            lambda **_: (_ for _ in ()).throw(
                AssertionError("expected-pnl gate must block before lifecycle launch")
            ),
        )
        summary = asyncio.run(
            launch_latest_stable_canary_once(
    ...
    assert summary.detail == (
        ...
        "expected_pnl=0.062000 < min=0.070000"
    )


def test_launch_latest_stable_canary_once_allows_launch_when_liquidity_and_value_requirements_met(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
        ...
        stable_canary_launch_min_daily_volume=50_000.0,
        stable_canary_launch_min_deployable_notional=500.0,
        stable_canary_launch_min_expected_one_day_round_trip_pnl=0.05,
    )
    approval, candidate, snapshot, stability = _build_stable_launch_test_snapshot(
        label="arb_extended_paradex",
        suggested_canary_notional=20.0,
    ...
    class StubLifecycleResult:
        def __init__(self) -> None:
            ...
            self.execution = ExecutionJournalEntry(
                ...
                status="submitted",
                paper_trade_id=119,
                preview_hash="launch-preview-liquidity-value",
                confirmation_entry_id=273,
                paper_trade=self.paper_trade,
                legs=[_build_auto_close_execution_leg()],
            )
            self.final_pair_status = _build_auto_close_pair_status(
...
def test_build_open_hedge_profit_protection_reason_flags_profit_giveback(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
        execution_auto_pair_close_enabled=True,
        execution_auto_pair_close_min_profit_total_collateral=0.05,
    ...
    reason = _build_open_hedge_profit_protection_reason(
        ...
        settings=settings,
        attribution=balance_service.summarize_paper_trade_attribution(21),
    )
...
def test_build_open_hedge_profit_protection_reason_ignores_small_giveback(
    tmp_path: Path,
) -> None:
    settings = WorkerSettings(
        database_path=str(tmp_path / "history.sqlite3"),
    ...
    reason = _build_open_hedge_profit_protection_reason(
        ...
        balance_service=balance_service,
        settings=settings,
        attribution=balance_service.summarize_paper_trade_attribution(23),
    )
...
def test_maybe_auto_close_open_hedged_execution_shadow_mode_logs_profit_giveback(
    tmp_path: Path,
    caplog: pytest.LogCaptureFixture,
) -> None:
    settings = WorkerSettings(
    ...
    execution = ExecutionJournalEntry(
        ...
        status="submitted",
        paper_trade_id=22,
        preview_hash="profit-giveback-preview",
        confirmation_entry_id=24,
        paper_trade=_build_auto_close_paper_trade(
            entry_id=22,
    ...
    assert "shadow auto-close for paper_trade_id=22" in caplog.text
    assert "profit giveback" in caplog.text
