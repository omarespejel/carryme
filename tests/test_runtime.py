...
def test_execution_quality_service_pages_recent_observations() -> None:
    ...
    class StubObservationStore:
        def __init__(self) -> None:
            ...
            self.entries = [
                ...
                make_observation(paper_trade_id=1, minute=13, outcome="cleanup_needed"),
                make_observation(paper_trade_id=2, minute=12, outcome="closed"),
                make_observation(paper_trade_id=3, minute=11, outcome="review_required"),
            ]
...
def test_opportunity_universe_service_reuses_overlaps_and_indexes_across_concurrent_scans() -> None:
    symbol_lists = {
        "extended": ["S-USD"],
        "paradex": ["S-USD-PERP"],
    }
    ...
    async def run() -> None:
        ...
        scan_one, scan_two, candidates = await asyncio.gather(
            service.scan(
                venues=["extended", "paradex"],
                ranking="route_adjusted_quality_pnl",
            ...
            service.scan_canary_candidates(
                venues=["extended", "paradex"],
                include_symbols=["S-USD-PERP"],
        ...
        assert len(scan_one.opportunities) == 1
        assert len(scan_two.opportunities) == 1
        assert len(candidates) == 1

    asyncio.run(run())

    assert symbol_calls == {"extended": 1, "paradex": 1}
    assert execution_quality_calls == 1
...
def test_execution_accounting_service_ignores_observation_without_strong_identity(
    tmp_path: Path,
) -> None:
    journal_store = ExecutionJournalStore(tmp_path / "history.sqlite3")
    observation_store = ExecutionObservationStore(tmp_path / "history.sqlite3")
    ...
    observation_store.append(
        ExecutionObservationEntry(
            ...
            order_state=ExecutionOrderState(
                ...
                legs=[
                    ExecutionLegOrderState(
                        venue="extended",
                        supported=True,
                        external_reference="carryme-pt11-extended-sell",
                        client_id="carryme-pt11-extended-sell",
                        derived_state="unknown",
                        notes=[
                            "Extended private API currently exposes open orders only; "
                            "absence here does not distinguish filled from closed."
            ...
            pair_status=ExecutionPairStatus(
                ...
                notes=["This observation is missing execution identity and must not infer fills."],
            ),
        )
    )
    ...
    assert summary.filled_leg_count == 1
    assert summary.legs[1].derived_fill_state == "unfilled"
    assert summary.legs[1].filled_notional is None
