...
import carryme_runtime.paradex_live_execution as paradex_live_execution
import httpx
import pytest
...
def test_build_live_submission_readiness_requires_confirmation_and_preflights() -> None:
    ...
    assert any(
        "CARRYME_API_PARADEX_BEARER_TOKEN" in reason for reason in readiness.blocking_reasons
    )
...
def test_order_preview_service_rejects_missing_known_venue_constraints() -> None:
    paper_trade = PaperTradeEntry(
        ...
        note="candidate accepted",
        intent=FundingPairTradeIntent(
            label="bad_extended_constraints",
            canonical_symbol="ARB-USD-PERP",
            source_recorded_at=datetime(2026, 3, 29, 12, 55, tzinfo=UTC),
            one_day_net_edge_after_entry=0.0008,
            break_even_days_entry=0.5,
    ...
    snapshots = {
        ("extended", "ARB-USD"): _snapshot(
            ...
            0.0921,
            25_000,
            raw={"tradingConfig": {"minOrderSize": "10"}},
        ),
        ("paradex", "ARB-USD-PERP"): _snapshot(
            "paradex",
            "ARB-USD-PERP",
    ...
    async def run() -> None:
        with pytest.raises(ValueError, match="Extended order constraints missing"):
            await OrderPreviewService(fetch_snapshot=fetch_snapshot).preview_paper_trade(
                paper_trade,
                slippage_tolerance_bps=10,
                generated_at=datetime(2026, 3, 29, 13, 5, tzinfo=UTC),
...
def test_order_preview_service_rejects_malformed_paradex_constraints() -> None:
    paper_trade = PaperTradeEntry(
        entry_id=11,
        created_at=datetime(2026, 3, 29, 13, 0, tzinfo=UTC),
        note="candidate accepted",
        intent=FundingPairTradeIntent(
            label="bad_paradex_constraints",
            canonical_symbol="ARB-USD-PERP",
            source_recorded_at=datetime(2026, 3, 29, 12, 55, tzinfo=UTC),
            one_day_net_edge_after_entry=0.0008,
            break_even_days_entry=0.5,
...
def test_build_signed_paradex_order_payload_rejects_over_precision() -> None:
    with pytest.raises(
        ConnectorError,
        match="Paradex order payload field size exceeds 8 decimal places",
    ):
        build_signed_paradex_order_payload(
            account_address="0x123",
            private_key="0x456",
...
def test_paradex_live_coerce_int_handles_negative_strings() -> None:
    ...
    assert paradex_live_execution._coerce_int("7") == 7
    assert paradex_live_execution._coerce_int("nope") is None
...
def test_paradex_live_execution_service_submits_confirmed_preview(
    ...
    async def run() -> None:
        ...
        assert entry.legs[0].signature_timestamp_ms is not None

    asyncio.run(run())


def test_paradex_live_execution_service_records_network_failures(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class StubTokenProvider:
        async def fetch_system_config(
    ...
    async def run() -> None:
        ...
        assert entry.legs[0].simulated is False
        assert entry.legs[0].external_reference == "carryme-pt7-paradex-buy"
        assert entry.legs[0].request_payload is not None
        assert entry.legs[0].response_payload is not None
        assert entry.legs[0].response_payload["error_type"] == "ReadTimeout"
        assert entry.legs[0].signature_timestamp_ms is not None
...
def test_extended_live_execution_service_submits_confirmed_cleanup_preview(
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path == "/api/v1/user/order":
            ...
            return httpx.Response(
                200,
                json={"status": "OK", "data": {"id": 654, "externalId": payload["id"]}},
            )
        raise AssertionError(f"Unexpected request path: {request.url.path}")
    ...
    class StubService:
        ...
        async def submit_confirmed_preview(
            ...
            *,
            ...
            leg = next(
                item
                for item in confirmation.preview.legs
                if item.venue.strip().lower() == self.venue
            )
...
def test_paired_live_execution_coordinator_rejects_duplicate_preview_venues() -> None:
    service = PairedLiveExecutionCoordinator(services={})
    paper_trade = PaperTradeEntry(
        entry_id=13,
        created_at=datetime(2026, 3, 29, 13, 0, tzinfo=UTC),
        note="candidate accepted",
    ...
    async def run() -> None:
        with pytest.raises(ValueError, match="exactly two distinct preview legs"):
            await service.submit_confirmed_preview(
                paper_trade=paper_trade,
                confirmation=confirmation,
                first_venue="extended",
...
def test_extended_account_probe_preserves_zero_account_balances(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/api/v1/user/account/info":
    ...
    async def run() -> None:
        ...
        assert status.authenticated is True
        assert status.total_collateral == pytest.approx(0.0)
        assert status.available_to_trade == pytest.approx(0.0)
