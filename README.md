...
> worker watchlist polling with persisted SQLite history, and API endpoints for
> health, fee references, live funding opportunities, and recent history.

The current workspace is intentionally narrow:
- `apps/api`: FastAPI endpoints for health, fee references, live scoring, and history
- `apps/worker`: worker CLI for deterministic health output and watchlist polling
